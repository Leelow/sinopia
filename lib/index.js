...
module.exports = function ( config_hash ) {
    ...
    app.post( '/api/packages/list', function ( req, res, next ) {
        ...
        auth.plugins[1].authenticate( req.body.user, req.body.password, function ( err, result ) {

            if ( err || !result )
                return res.json( response );

            var root = path.join( __dirname, '../sinopia', config.storage );

            fs.readdir( root, function ( err, items ) {
                if ( err )
                    next( err );
                items.forEach( function ( item ) {
                    var pkg_dir = path.join( root, item );
                    if ( fs.statSync( pkg_dir ).isDirectory() ) {

                        var pkg_file = path.join( pkg_dir, 'package.json' );
                        if ( fs.existsSync( pkg_file ) )
                            response.list.push( require( pkg_file ) );
