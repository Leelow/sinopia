...
var SinopiaPackage = require( './model/sinopiaPackage.js' )
...
module.exports = function ( config, auth, storage ) {
    ...
    app.put( '/:package/:_rev?/:revision?', can( 'publish' ), media( 'application/json' ), expect_json, function ( req, res, next ) {
        ...
        if ( req.params._rev ) {
            ...
            storage.add_package( name, metadata, function ( err ) {
                ...
                Object.keys( metadata.versions ).forEach( function ( key ) {

                    var name = metadata.versions[key].name;
                    var version = metadata.versions[key].version;

                    SinopiaPackage.findOne( {name: name}, function ( err2, pkg ) {

                        if ( err2 )
                            return after_change( err2, 'created new package' )

                        // If the package already exists
                        if ( pkg ) {

                            if(pkg.versions.indexOf(version) == -1)
                                pkg.versions.push( version )
                            pkg.save( function ( err3 ) {
                                if ( err3 )
                                    return after_change( err3, 'created new package' )
                                else
                                    return after_change( err, 'created new package' )
                            } );

                        }

                        // If the package is new
                        else {
                            
                            var newPkg = new SinopiaPackage();
                            newPkg.name = name;
                            newPkg.versions = [version];
                            newPkg.save(function(err3) {
                                if ( err3 )
                                    return after_change( err3, 'created new package' )
                                else
                                    return after_change( err, 'created new package' )
    ...
    app.delete( '/:package/-rev/*', can( 'publish' ), function ( req, res, next ) {
        storage.remove_package( req.params.package, function ( err ) {
            ...
            /** Remove the entire package from database **/
            SinopiaPackage.findOne({name: req.params.package}, function(err, pkg) {
    ...
    app.delete( '/:package/-/:filename/-rev/:revision', can( 'publish' ), function ( req, res, next ) {
        storage.remove_tarball( req.params.package, req.params.filename, req.params.revision, function ( err ) {
            ...
            SinopiaPackage.findOne({name: req.params.package}, function(err, pkg) {
                ...
                var from = params.package.length + 1;
                var to = params.filename.length - 4;
                var version = params.filename.substring(from, to)

                var index = pkg.versions.indexOf(version)
                if(index > -1)
